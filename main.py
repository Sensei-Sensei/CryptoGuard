...
class CryptoScreen(MDScreen):
    def __init__(self, **kwargs):
        ...
        # UI
        layout.add_widget(MDLabel(text="CRYPTO GUARD", halign="center", font_style="H5", bold=True, theme_text_color="Custom", text_color=(0, 0.8, 1, 1), size_hint_y=None, height=dp(50)))

        self.card = MDCard(orientation='vertical', padding=dp(15), spacing=dp(15), elevation=0, radius=[20,], md_bg_color=(1, 1, 1, 0.05), size_hint_y=None, height=dp(420), line_color=(1, 1, 1, 0.1))

        self.msg_input = MDTextField(hint_text="Mensagem", mode="rectangle", multiline=True, keyboard_suggestions=False)
        self.pwd_input = MDTextField(hint_text="Chave", password=True, mode="rectangle", keyboard_suggestions=False)
    ...
    def update_ui(self, result, message, is_error=False):
        self.res_label.text = result if not is_error else "Erro."
        if not is_error:
            self.last_result = result
            Clipboard.copy(result)
        self.show_toast(message, (0.8, 0.2, 0.2, 1) if is_error else (0, 0.8, 1, 1))

    # EXECUÇÃO EM BACKGROUND PARA NÃO CRASHAR
    def start_encrypt(self, *args):
        threading.Thread(target=self._encrypt_logic).start()
    ...
    def _encrypt_logic(self):
        ...
        try:
            ...
            self.update_ui(final, "Protegido e Copiado!")
        except:
            self.update_ui("", "Falha crítica", True)

    def start_decrypt(self, *args):
        threading.Thread(target=self._decrypt_logic).start()
